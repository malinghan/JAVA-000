...
public class RpcClientByteBuddy implements RpcClient {
    ...
    public <T> T create(final Class<T> serviceClass, final String url, final Filter... filters) {
        return (T) new ByteBuddy().subclass(Object.class).implement(serviceClass).intercept(InvocationHandlerAdapter.of(new RpcInvocationJDKHandler(serviceClass, url))).make().load(RpcClientByteBuddy.class.getClassLoader()).getLoaded().getDeclaredConstructor().newInstance();
    }
